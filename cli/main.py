...
# Research-stage agents in execution order. Single source of truth for the
# status table, the display layout, and the bulk status transitions, which
# previously each carried their own copy of the list.
RESEARCH_TEAM = [
    "Buffett Researcher",
    "Bull Researcher",
    "Bear Researcher",
...
class MessageBuffer:
    ...
    def init_for_analysis(self, selected_analysts, buffett_enabled=True):
        """Initialize agent status and report sections based on selected analysts.

        Args:
            selected_analysts: List of analyst type strings (e.g., ["market", "news"])
            buffett_enabled: Whether the graph includes the Buffett Researcher.
                When False it is left out of the status table entirely, since a
                row that never leaves "pending" reads as a stall.
        """
        ...
        for team_agents in self.FIXED_AGENTS.values():
            for agent in team_agents:
                if agent == "Buffett Researcher" and not buffett_enabled:
                    continue
...
def get_user_selections():
    ...
    if provider_from_env:
        ...
        codex_reasoning_effort = thinking_value_or_prompt(
            "TRADINGAGENTS_CODEX_REASONING_EFFORT",
            "codex_reasoning_effort",
            "Codex reasoning effort",
            "Step 8: Codex Reasoning Effort",
            "Configure Codex subscription reasoning effort",
            ask_codex_reasoning_effort,
        )
...
def display_complete_report(final_state):
    ...
    if final_state.get("investment_debate_state") or final_state.get("buffett_report"):
        debate = final_state.get("investment_debate_state") or {}
        research = []
        if final_state.get("buffett_report"):
            research.append(("Buffett Researcher", final_state["buffett_report"]))
...
def update_research_team_status(status):
    ...
    Never walks an agent back from "completed": the Buffett Researcher
    finishes before the debate starts, and the debate's bulk transition would
    otherwise show it as running again.
    """
    for agent in research_team_agents(message_buffer):
        if message_buffer.agent_status.get(agent) == "completed":
            continue
        message_buffer.update_agent_status(agent, status)


def research_team_agents(buffer):
    """Research agents actually running this session, in execution order."""
    return [agent for agent in RESEARCH_TEAM if agent in buffer.agent_status]
...
def update_analyst_statuses(message_buffer, chunk, wall_time_tracker=None):
    ...
    - When all analysts done, start the first research agent
    """
    ...
    research_agents = research_team_agents(message_buffer)
    if (
        not found_active
        and selected
        and research_agents
        and message_buffer.agent_status.get(research_agents[0]) == "pending"
    ):
        message_buffer.update_agent_status(research_agents[0], "in_progress")
...
def run_analysis(checkpoint: bool | None = None):
    ...
    message_buffer.init_for_analysis(
        selected_analyst_keys,
        buffett_enabled=bool(config.get("buffett_researcher_enabled", True)),
    )
    ...
    with Live(layout, refresh_per_second=4):
        ...
        for chunk in graph.graph.stream(init_agent_state, **args):
            ...
            # Research Team - Buffett Researcher runs first, ahead of the debate
            if chunk.get("buffett_report"):
                message_buffer.update_report_section(
                    "investment_plan",
                    f"### Buffett Researcher Assessment\n{chunk['buffett_report']}",
                )
                if message_buffer.agent_status.get("Buffett Researcher") != "completed":
                    message_buffer.update_agent_status("Buffett Researcher", "completed")
